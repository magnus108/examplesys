...
data Time = Time
  { _view :: UI.Element,
    _userTE :: UI.Tidings Time.UTCTime,
...
userText :: Time -> UI.Tidings Time.UTCTime
userText = _userTE

errText :: Time -> UI.Tidings (Maybe String)
errText = _errTE

setup :: Monad.AppEnv -> R.Behavior (Maybe String) -> R.Behavior Time.UTCTime -> UI.UI Time
setup env berr bsucc = mdo
  textEntry <- UI.entry (Time.formatTime Time.defaultTimeLocale "%F, %T" <$> bsucc)
  ...
  eParse <- liftIO $ Monad.runApp env $ UnliftIO.withRunInIO $ \run -> do
    let e = R.unsafeMapIO (run . UnliftIO.tryAny . Time.parseTimeM True Time.defaultTimeLocale "%F, %T") eEntry
    return e
